import { Pencil, Plus, Save, Siren, Trash2, X } from 'lucide-react'
import { useState } from 'react'
import { OUTLETS, type IncidentItem } from '../types'
import { Badge, type BadgeTone } from './Badge'

let seq = 0
const nextId = () => `inc-${++seq}-${Date.now().toString(36)}`

const STATUSES: IncidentItem['statusPenanganan'][] = [
  'Proses Investigasi',
  'Selesai',
  'Tindak Lanjut',
]

const STATUS_TONE: Record<IncidentItem['statusPenanganan'], BadgeTone> = {
  'Proses Investigasi': 'accent',
  Selesai: 'ok',
  'Tindak Lanjut': 'blue',
}
...
export function ModuleIncident({ items, onChange }: Props) {
  const [lokasi, setLokasi] = useState<string>(OUTLETS[0].code)
  const [jenis, setJenis] = useState('')
  const [status, setStatus] = useState<IncidentItem['statusPenanganan']>('Proses Investigasi')
  const [editingId, setEditingId] = useState<string | null>(null)
  const [draft, setDraft] = useState<IncidentItem | null>(null)

  const canAdd = jenis.trim() !== ''
  const add = () => {
    if (!canAdd) return
    onChange([
      ...items,
      { id: nextId(), lokasi, jenisKejadian: jenis.trim(), statusPenanganan: status },
    ])
    setJenis('')
    setStatus('Proses Investigasi')
  }

  const startEdit = (item: IncidentItem) => {
    setEditingId(item.id)
    setDraft({ ...item })
  }
  const saveEdit = () => {
    if (!draft || !draft.jenisKejadian.trim()) return
  ...
  return (
    <section className="panel flex min-h-0 flex-col p-4 sm:p-5">
      <header className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="label">Modul 2</p>
          <h2 className="mt-1 font-display text-base font-semibold tracking-wide text-hi">
            Incident Report
          </h2>
          <p className="mt-0.5 text-xs text-mid">
            Catat kejadian di lapangan beserta status penanganannya.
          </p>
        </div>
        <Badge tone="bad">{items.length} kejadian</Badge>
      </header>

      <div className="mb-4 grid gap-2 sm:grid-cols-[180px_minmax(0,1fr)_auto_auto]">
        <div>
          <label htmlFor="inc-lokasi" className="label mb-1.5 block">
            Lokasi
          </label>
          <select
            id="inc-lokasi"
        ...
        <div>
          <label htmlFor="inc-jenis" className="label mb-1.5 block">
            Jenis Kejadian
          </label>
          <input
            id="inc-jenis"
            ...
            placeholder="cth: Pencurian di pos, kerusakan CCTV…"
            className="input"
          />
        </div>
        <div>
        ...
        <table className="w-full min-w-[680px] text-left">
          <thead>
            <tr className="border-b border-line text-[10px] uppercase tracking-[0.14em] text-low">
              ...
              <th className="py-2 pr-3 font-semibold">Jenis Kejadian</th>
              <th className="py-2 pr-3 font-semibold">Status Penanganan</th>
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={5} className="py-10 text-center">
                  <Siren className="mx-auto mb-2 h-6 w-6 text-low" />
                  <p className="text-xs text-low">Belum ada kejadian tercatat.</p>
                </td>
              </tr>
            )}
            {items.map((it, i) =>
              editingId === it.id ? (
                <tr key={it.id} className="bg-ink-850/60">
                  ...
                  <td className="py-2 pr-3">
                    <input
                      value={draft?.jenisKejadian ?? it.jenisKejadian}
                      onChange={(e) =>
                        setDraft((d) => (d ? { ...d, jenisKejadian: e.target.value } : d))
                      }
                      className="input px-2 py-1.5 text-xs"
                    />
                  </td>
                  <td className="py-2 pr-3">
                    <select
                      value={draft?.statusPenanganan ?? it.statusPenanganan}
                      onChange={(e) =>
                        setDraft((d) =>
                          d
                            ? {
                                ...d,
                                statusPenanganan: e.target
                                  .value as IncidentItem['statusPenanganan'],
                              }
                            : d,
                        )
                      }
                ...
                <tr key={it.id} className="transition-colors hover:bg-ink-850/40">
                  ...
                  <td className="py-2.5 pr-3 text-xs text-mid">{it.jenisKejadian}</td>
                  <td className="py-2.5 pr-3">
                    <Badge tone={STATUS_TONE[it.statusPenanganan]}>{it.statusPenanganan}</Badge>
                  </td>
                  <td className="py-2.5">
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        ...
                        aria-label={`Edit kejadian ${it.jenisKejadian}`}
                      ...
                      <button
                        ...
                        aria-label={`Hapus kejadian ${it.jenisKejadian}`}
