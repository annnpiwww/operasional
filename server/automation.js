...
export async function runSpreadsheetAutomation(params, logCallback = () => {}) {
  ...
  try {
    ...
    log(`Inserting income Rp ${income} for outlet ${location} (Data Kemarin: ${yesterdayStr}, Komparasi Minggu Lalu: ${priorWeekStr})...`);
    ...
    return { success: true, params, yesterdayStr, priorWeekStr };
  } catch (err) {
...
export async function runAllOutletsAutomation(params, logCallback = () => {}) {
  ...
  try {
    ...
    const incomes = OUTLETS.map((outlet, index) => {
      ...
      const hariIni = baseIncome + randomDiff; // Represents Yesterday's income
      const mingguLalu = baseIncome; // Represents 7 days prior to Yesterday
